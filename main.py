    ...
    while True:
        ...
        if class_choice in class_map:
            ...
            try:
                ...
                # create_character should return a character dict
                if current_character is None:
                    print("Failed to create character (create_character returned None).")
                    return
    ...
    while True:
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(saved_characters):
            ...
            try:
                ...
                if current_character is None:
                    print("Failed to load character (load_character returned None).")
                    return
...
def view_character_stats():
    ...
    # Defensive access in case keys are missing
...
def explore():
    ...
    # If character lacks level, treat as level 1
    level = current_character.get('level', 1)
    enemy_levels = [level, level + 1]
    enemy_level = random.choice(enemy_levels)

    # Use combat_system function to create a random enemy; fall back to create_enemy if missing
    try:
    ...
    print(f"\nA wild {name} (Level {enemy.get('level', '?')}) appears!")
    battle = SimpleBattle(current_character, enemy)
    try:
        result = battle.start_battle()
        # support both dict result or legacy 'victory'/'defeat' strings
        ...
        if winner == 'player' or winner == 'victory':
            ...
            try:
                ...
                # fallback to direct field manipulation if function missing
                current_character['experience'] = current_character.get('experience', 0) + xp_gained
            current_character['gold'] = current_character.get('gold', 0) + gold_gained
        ...
        print(f"An error occurred during combat: {e}")
...
def shop():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if item_data:
                try:
                    # inventory_system.purchase_item is expected; if absent use add_item_to_inventory and deduct gold
                    if hasattr(inventory_system, 'purchase_item'):
                        inventory_system.purchase_item(current_character, item_id, item_data)
                    else:
                        # fallback
                        if current_character.get('gold', 0) < item_data.get('cost', 0):
                        ...
                        current_character['gold'] -= item_data.get('cost', 0)
                    print(f"Purchased '{item_data.get('name')}' for {item_data.get('cost', 0)} gold.")
            ...
            if inventory:
                ...
                if item_data:
                    try:
                        ...
                        print(f"Sold '{item_data.get('name')}' for {sell_price} gold.")
...
def handle_character_death():
    ...
    while True:
        ...
        if choice == 'R':
            try:
                # revive_character may raise InsufficientGoldError
                character_manager.revive_character(current_character)
                print(f"{current_character.get('name', 'Character')} has been revived!")
                return
            except InsufficientGoldError:
                print("Not enough gold to revive!")
            except AttributeError:
                # fallback behavior if revive function not implemented
                cost = 10
